...
ReadAmplicon <- function(otu, tax, silva, domain,
  ...
  Tax0 <- read.table(tax, h = T, sep = ",", quote = "", comment.char = "", stringsAsFactors = )
  
  #check order of sequences
  accnos <- sapply(as.character(Tax0$name), function(x) {
    strsplit(x, "_", fixed = T)[[1]][1]
    }
    )
  names(accnos) <- NULL
  if (sum(accnos %in% as.character(Data0$amplicon)) != nrow(Data0)) {
    print("OTU_contigency_table and taxonomy do not match\n")
  } else {
    Tax0 <- Tax0[match(as.character(Data0$amplicon), accnos), ]
    Tax0$accnos <- accnos
    rownames(Tax0) <- rownames(Data0)
    Tax0$path <- Tax0$lca_tax_slv
    Tax0$align <- Tax0$align_quality_slv
    
    #removing unwanted lineages ()
    Data0a <- Data0[grep("Bacteria;|Archaea;", Tax0$path), ]
    Tax0a <- Tax0[grep("Bacteria;|Archaea;", Tax0$path), ]
    Data0b <- Data0a[-c(grep("[cC]hloroplast", Tax0a$path), grep("[mM]itochondria", Tax0a$path)), ]
    Tax0b <- Tax0a[-c(grep("[cC]hloroplast", Tax0a$path), grep("[mM]itochondria", Tax0a$path)), ]
    ...
    SILVA0 <- read.table(silva, sep = silva.sep, h=T) # h = T
    #colnames(SILVA0) <- c("path","taxid", "rank", "remark", "release") # Added this line for new version of the database, or add headers in the database
    SILVA <- SILVA0[grep("Bacteria;|Archaea;", SILVA0$path), ]
